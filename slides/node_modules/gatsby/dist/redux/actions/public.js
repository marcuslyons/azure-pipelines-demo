...
${reservedFields.map(f => `  * "${f}"`).join(`\n`)}
  ...
  if (!internalPage.component.includes(`/.cache/`) && process.env.NODE_ENV === `production` && !fileOkCache[internalPage.component]) {
    ...
    if (!fileContent.includes(`export default`) && !fileContent.includes(`module.exports`) && !fileContent.includes(`exports.default`) && !fileContent.includes(`exports["default"]`) && !fileContent.match(/export \{[\0-\uFFFF]* as default[\0-\uFFFF]*\}/) && // this check only applies to js and ts, not mdx
